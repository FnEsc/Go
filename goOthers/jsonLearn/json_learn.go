...
func json_learn() {
	...
	fmt.Println(string(res2B)) // {"page":1,"my_fruits":["apple","peach","pear"]}

	byt := []byte(`{"num":6.13,"strs":["a","b"]}`)

	var dat map[string]interface{}

	if err := json.Unmarshal(byt, &dat); err != nil { // Unmarshal 要符合格式
		panic(err)
	}
	...
	str := `{"page": 1, "my_fruits": ["apple", "peach"]}`
	res := response2{}
	...
	d := map[string]int{"apple": 5, "lettuce": 7, "中文": 8}
